...
class LeanCloudVerifyHandler(B64Handler):
    ...
    def get(self):
        ...
        if not isinstance(phone,bytes):
            phone = phone.encode()
        try:
            phone = base64.urlsafe_b64decode(phone).decode('utf8')
        except:
            phone = None
            self.write_error("phone number not correct")
        if not phone:
            self.write_error("argument error")
            return

        old_user = self.session.query(User).filter(User.phone==phone)
        if old_user.count():
            self.write_error("user exist", 403)
        else:
